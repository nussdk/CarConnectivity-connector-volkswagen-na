    ...
    def fetch_vehicle_status(self, vehicle: VolkswagenNAVehicle) -> None:
        ...
        if data is not None:
            ...
            if isinstance(vehicle, VolkswagenNAElectricVehicle):
                ...
                if (
                    ...
                    if "climateStatusReport" in climate_data and climate_data["climateStatusReport"] is not None:
                        ...
                        if "climateStatusInd" in climatisation_status and climatisation_status["climateStatusInd"] is not None:
                            if climatisation_status["climateStatusInd"] in [item.value for item in VolkswagenClimatization.ClimatizationState]:
                                climatization_state: VolkswagenClimatization.ClimatizationState = VolkswagenClimatization.ClimatizationState(
                                    climatisation_status["climateStatusInd"]
                                )
                            else:
                                LOG_API.info(
                        ...
                        log_extra_keys(
                            LOG_API, "climateStatusReport", climatisation_status, {"carCapturedTimestamp", "climateStatusInd", "remainingClimatisationTimeMin"}
                        )
                    else:
            ...
            log_extra_keys(
                ...
                "selectivestatus",
                data,
                {"measurements", "access", "vehicleLights", "climatisation", "vehicleHealthInspection", "charging", "readiness"},
            )
