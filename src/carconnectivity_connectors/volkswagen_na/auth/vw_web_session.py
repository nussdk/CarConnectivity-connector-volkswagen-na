...
class VWWebSession(OpenIDSession):
            ...
            if "terms-and-conditions" in url:
                if self.accept_terms_on_login:
                    ...
                    raise AuthenticationError(
                        f'It seems like you need to accept the terms and conditions. Try to visit the URL "{url}" or log into smartphone app.'
                    )
    ...
    def _get_login_form(self, url: str) -> HTMLFormParser:
        while True:
            ...
            raise APICompatibilityError(f"Retrieving login page was not successful, status code: {response.status_code}")
    ...
    def _handle_login(self, url: str, data: Dict[str, Any]) -> str:
        ...
        if response.status_code not in (requests.codes["found"], requests.codes["see_other"]):
            raise APICompatibilityError(f"Forwarding expected (status code 302), but got status code {response.status_code}")
