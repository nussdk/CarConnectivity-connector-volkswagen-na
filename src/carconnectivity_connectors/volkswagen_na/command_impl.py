...
class SpinCommand(GenericCommand):
    ...
    def value(self, new_value: Optional[Union[str, Dict]]) -> None:
        ...
        if isinstance(new_value, SpinCommand.Command):
            ...
            parser.add_argument("command", help="Command to execute", type=SpinCommand.Command, choices=list(SpinCommand.Command))
            parser.add_argument("--spin", dest="spin", help="Spin to be used instead of spin from config or .netrc", type=str, required=False, default=None)
            try:
            ...
            if "command" in new_value and isinstance(new_value["command"], str):
                if new_value["command"] in SpinCommand.Command:
                    ...
                    raise ValueError(f"Invalid value for SpinCommand. Command must be one of {SpinCommand.Command}")
